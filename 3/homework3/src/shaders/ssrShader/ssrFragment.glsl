...
vec3 toScreenSpace(vec3 pos){
  vec3 coord = vec3(GetScreenCoordinate(pos), GetDepth(pos)); // all 0...1
  return coord;
}

float GetGBufferDepthLOD(vec2 uv, float lod) {
  float depth = texture2D(uGDepth, uv, lod).x;
  if (depth < 1e-2) {
    depth = 1000.0;
  }
  return depth;
...
vec3 EvalIndirectLight(vec3 pos){
  ...
  for(int i = 0; i < SAMPLE_NUM;i++){
    ...
    if(RayMarch(pos, dir, hitPos)){
      ...
      //L *= EvalDirectionalLight(GetScreenCoordinate(hitPos));
      Li += L;
    }
  }
...
void main() {
  ...
  L += Li;
  //if(fract(vPosWorld.x)<0.01 || fract(vPosWorld.z)<0.01)
  //  L.r = 1.0;
  L = vec3(GetGBufferDepth(GetScreenCoordinate(vPosWorld.xyz)));
  ...
  bool intersect = RayMarch(vPosWorld.xyz, dir, hitPos);
  if(intersect){
    color = GetGBufferDiffuse(GetScreenCoordinate(hitPos)) * 0.5 + color * 0.5;
  }*/
  gl_FragColor = vec4(color, 1.0);
  //vec3 normal = GetGBufferNormalWorld(GetScreenCoordinate(vPosWorld.xyz));
  //gl_FragColor = vec4(vPosWorld.xyz, 1.0);
}
