...
namespace ProjEnv
{
                                                    ...
                                                    const int &channel)
    {
        ...
        //sh::SphericalFunction f1 = [](double a, double b)->double {return 0.5; };
        //auto cof = sh::ProjectFunction(SHOrder, f1, 10000);
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l = 0; l <= SHOrder; l++){
                        for (int m = -l; m <= l; m++){
                            float sh = sh::EvalSH(l, m, phi, theta), delta = CalcArea((float)x, (float)y, width, height);
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                else
                {
                    ...
                    if (H > 0.0){
                        ...
                        bool intersect = scene->rayIntersect(ray);
                        if(intersect)
                            return 0.0;
                        return H;
                    }
