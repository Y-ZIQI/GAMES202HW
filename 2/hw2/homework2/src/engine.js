...
let frame = 0;
let rotx = 0.0, rm = mat4.create();
/* if this set true, model will rotate along Y axis */
let rotating = true;
/* if this set true, transport file name will be "/transport_*.txt" instead of "/transport.txt", which * can be "unshadowed"/"shadowed"/"interreflection" */
/* then you can configure guiParams.sdtypeId to change the Light Transport mode */
let use_my_transport_file_name = false;
...
async function GAMES202Main() {
	...
	let transFile;
	if(use_my_transport_file_name)
		transFile = "/transport_"+sdtype[guiParams.sdtypeId]+".txt";
	else
		transFile = "/transport.txt";
	// file parsing
	for (let i = 0; i < envmap.length; i++) {
		let val = '';
		await this.loadShaderFile(envmap[i] + transFile).then(result => {
	 		val = result;
	 	});
	...
	loadOBJ(renderer, 'assets/mary/', 'mary', 'DiffuseMaterial', box2Transform);
	...
	function mainLoop(now) {
		...
		if(renderer.meshes.length > 1){
			mat4.identity(rm);
			if(rotating){
				mat4.rotateY(rm, rm, rotx);
				rotx += 0.01;
			}
			let rL = getRotationPrecomputeL(precomputeL[guiParams.envmapId], rm);
			renderer.meshes[1].material.setPrecomputeL(rL);
			renderer.meshes[1].material.setUniform('uRotateMatrix', rm);
			rotx += 0.01;
			frame += 1;
			//renderer.meshes[1].material.checkEnvmap();
		}
	}
	requestAnimationFrame(mainLoop);
}
